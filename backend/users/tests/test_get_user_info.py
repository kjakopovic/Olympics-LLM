...
from GetUserInfo.lambda_handler import lambda_handler

@mock_aws
class TestGetUserInfoLambda(BaseUsersTest):
    def setUp(self):
        super().setUp()
    ...
    def test_success(self):
        """
        Test response when users info is retrieved successfully.
        """
        ...
        self.assertEqual(body['message'], "Info retrieved successfully")
        self.assertIn("info", body)
        self.assertIn("legal_name", body['info'])
        self.assertIn("email", body['info'])
        self.assertIn("phone_number", body['info'])
        self.assertIn("tags", body['info'])